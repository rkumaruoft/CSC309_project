...
export default function Home() {
  ...
  // npm run dev for DEV mode. Otherwise npm run preview
  const demoUser = import.meta.env.DEV ? { utorid: 'demo', name: 'Demo User', points: 1250 } : undefined;
  ...
  // If there is no authentication token (and no DEV user), redirect back to the login page. 
  useEffect(() => {
    const token = localStorage.getItem('token');
